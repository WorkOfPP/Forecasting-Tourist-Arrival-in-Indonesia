...
**BOXCOX TRANSFORMATIONS** Apply boxcox transformation for all variables. We see from previous tsdisplay plots, their variances are not stable.
...
*LINEAR MODEL(REGRESSION)* Use the linear model with time series components function tslm() to fit a linear model to the log(arrival) time series as a function of the log(oil) time series (i.e., Oil -\> Independent variable, Arrival -\> Dependent variable).
...
```{r}
mse_3 <- mean(forecast_arima_1$residuals^2)
print(mse_3)
```
...
```{r}
arima_model_3 <- auto.arima(arrival_ts, xreg = cbind(oil_ts, geopol_ts, globcci_ts), lambda = 0, seasonal = TRUE, D = 1, trace = TRUE)
```
...
```{r}
forecast_arima_3 <- forecast(arima_model_3, level=c(80,95), h=48, xreg=cbind(oil_forecast$mean, geopol_forecast$mean, globcci_forecast$mean))
```

```{r}
autoplot(forecast_arima_3)
```

```{r}
accuracy(forecast_arima_3, test_data$arrival)
```

```{r}
arima_model_4 <- auto.arima(arrival_ts, xreg =globcci_ts, lambda = 0, seasonal = TRUE, D = 1, trace = TRUE)
```
...
```{r}
forecast_arima_4 <- forecast(arima_model_4, level=c(80,95), h=48, xreg=globcci_forecast$mean)
autoplot(forecast_arima_4)
```

```{r}
accuracy(forecast_arima_4, test_data$arrival)
```
