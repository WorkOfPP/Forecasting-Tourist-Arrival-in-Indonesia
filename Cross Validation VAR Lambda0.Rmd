...
for(i in 1:(n-k))
{
  ...
  mape_1[i,1:length(test_arrival)] <- (mean(abs(fcast_1 - test_arrival)) / test_arrival) * 100
  rmse_1[i,1:length(test_arrival)] <- sqrt(mean((fcast_1 - test_arrival)^2))

  # Calculate MAE and RMSE for Sliding Window
  mape_2[i,1:length(test_arrival)] <- (mean(abs(fcast_2 - test_arrival)) / test_arrival) * 100
  rmse_2[i,1:length(test_arrival)] <- sqrt(mean((fcast_2 - test_arrival)^2))
  
  # Calculate MAE and RMSE for Sliding Window
  mape_3[i,1:length(test_arrival_l)] <- (mean(abs(fcast_3 - test_arrival_l)) / test_arrival_l) * 100
  rmse_3[i,1:length(test_arrival_l)] <- sqrt(mean((fcast_3 - test_arrival_l)^2))
  
  # Calculate MAE and RMSE for Sliding Window
  mape_4[i,1:length(test_arrival_l)] <- (mean(abs(fcast_4 - test_arrival_l)) / test_arrival_l) * 100
