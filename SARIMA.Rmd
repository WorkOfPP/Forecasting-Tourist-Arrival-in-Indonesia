...
##MODEL TRAINING 1. SARIMA model 1 with auto.arima function
...
```{r}
sarima_model_3 <- auto.arima(train_ts, trace=TRUE, seasonal=TRUE, lambda=0)
```

```{r}
summary(sarima_model_3)
```

```{r}
checkresiduals(sarima_model_3)
```
...
identify seasonal and non seasonal behavior: Non-seasonal behavior: ACF cut off at lag 1, PACF slowly decay -\> (1,1,0) seasonal behavior: ACF significant at lag 12, PACF significant at lag 12 -\> (1,1,1)
...
#SARIMA(1,1,0)(1,1,1)[12]
#Apply box cox transformation first
transformed_data_2 <- BoxCox(train_ts, lambda=0)
sarima_model_4 <- Arima(transformed_data_2, order = c(1, 1, 0), 
                          seasonal = list(order = c(1, 1, 1), period = 12))
summary(sarima_model_4)
```

```{r}
checkresiduals(sarima_model_4)
```

*Forecast* 1. Forecast sarima model 1
...
```{r}
mse <- mean(forecast_sarima_1$residuals^2)
print(mse)
```

```{r}
forecast_sarima_3 <- forecast(sarima_model_3, h=48)

# Combine forecast and test data
combined_data <- c(as.vector(train_ts), as.vector(forecast_sarima_3$mean))
...
# Plot
autoplot(combined_ts) +
  autolayer(forecast_sarima_3, series = "Forecast") +
  autolayer(test_ts, series = "Actual")
```

```{r}
accuracy(forecast_sarima_3, test_data$arrival)
```
...
Forecast sarima model 4

```{r}
forecast_sarima_4 <- forecast(sarima_model_4, h=48)

lambda <- 0
forecast_inv <- bxcx(forecast_sarima_4$mean, lambda=lambda, InverseQ=TRUE)
